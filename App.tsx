...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'buyers':
        return <CreditManagement buyers={buyers} transactions={transactions} selectedBranch={selectedBranch} onUpdateBuyer={(b) => setBuyers(prev => prev.map(old => old.id === b.id ? b : old))} onAddBuyer={(b) => setBuyers(prev => [...prev, b])} />;
      case 'suppliers':
  ...
  if (isInitialLoading) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center font-['Inter']">
        ...
        <p className="text-slate-500 text-[10px] font-bold uppercase mt-2 tracking-widest">Handshaking with Neon PostgreSQL...</p>
      </div>
    );
  }
  ...
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 font-['Inter'] relative overflow-hidden">
        ...
        <div className="w-full max-w-2xl bg-white rounded-[4rem] p-12 shadow-2xl relative z-10 animate-in fade-in zoom-in-95 duration-500">
          <div className="flex justify-between items-start mb-12">
            <div>
              <div className="w-20 h-20 bg-blue-600 rounded-[2rem] flex items-center justify-center mb-6 shadow-2xl shadow-blue-600/30">
                <Building2 className="text-white" size={40} />
              </div>
              <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase">OSAKA <span className="text-blue-600">ERP</span></h1>
              <p className="text-slate-500 font-bold text-xs uppercase tracking-[0.4em] mt-2">Enterprise Access Shield</p>
            </div>
          ...
          {loginStep === 'ROLE' ? (
            <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
              <div className="grid grid-cols-1 gap-4">
                <button onClick={() => handleRoleSelection(UserRole.SUPER_ADMIN)} className="w-full p-8 bg-slate-950 text-white rounded-[2.5rem] font-black text-sm hover:bg-blue-600 transition-all flex items-center justify-between group shadow-xl">
                  <div className="flex items-center gap-6">
                    <div className="p-4 bg-white/10 rounded-2xl"><ShieldCheck size={24} /></div>
                    <div className="text-left"><span className="block text-base uppercase tracking-widest">Super Admin</span><span className="text-[10px] text-slate-400 font-bold uppercase">Master Clearance</span></div>
                  </div>
                  <ChevronRight size={20} className="group-hover:translate-x-1 transition-transform" />
                </button>
                <button onClick={() => handleRoleSelection(UserRole.BRANCH_ADMIN)} className="w-full p-8 bg-slate-50 text-slate-900 rounded-[2.5rem] font-black text-sm hover:bg-slate-100 transition-all flex items-center justify-between group">
                  <div className="flex items-center gap-6">
                    <div className="p-4 bg-blue-600/10 text-blue-600 rounded-2xl"><UserIcon size={24} /></div>
                    <div className="text-left"><span className="block text-base uppercase tracking-widest">Branch Admin</span><span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Shop Manager</span></div>
                  </div>
                  <ChevronRight size={20} />
                </button>
                <button onClick={() => handleRoleSelection(UserRole.STAFF)} className="w-full p-8 bg-slate-50 text-slate-900 rounded-[2.5rem] font-black text-sm hover:bg-slate-100 transition-all flex items-center justify-between group">
                  <div className="flex items-center gap-6">
                    <div className="p-4 bg-emerald-600/10 text-emerald-600 rounded-2xl"><Store size={24} /></div>
                    <div className="text-left"><span className="block text-base uppercase tracking-widest">Staff / Cashier</span><span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Terminal Mode</span></div>
                  </div>
                  <ChevronRight size={20} />
                </button>
              </div>
             ...
             <div className="space-y-6 animate-in slide-in-from-right-10 duration-500">
                <div className="mb-4">
                  ...
                  <h2 className="text-2xl font-black text-slate-900">Select Operating Node</h2>
                  <p className="text-xs font-bold text-blue-600 uppercase tracking-widest mt-1">Authorized role: {role?.replace('_', ' ')}</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                   {Object.values(Branch).filter(b => b !== Branch.ALL).map(b => (
                      <button 
                        ...
                        className="p-6 bg-slate-50 border-2 border-slate-100 hover:border-blue-600 hover:bg-white rounded-[2rem] text-left transition-all group"
                      >
                         <div className="w-10 h-10 bg-white rounded-xl shadow-sm flex items-center justify-center mb-4 group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors">
                            <MapPin size={18} />
                         </div>
                         <p className="text-xs font-black uppercase tracking-tight text-slate-900 leading-tight">{b}</p>
                         <p className="text-[8px] font-bold text-slate-400 uppercase mt-1">Osaka Network Terminal</p>
                      </button>
                   ))}
                </div>
             </div>
          ) : (
            <form onSubmit={handleLoginAttempt} className="space-y-8 animate-in slide-in-from-right-10 duration-500">
              <div className="mb-4">
                ...
                <h2 className="text-2xl font-black text-slate-900">Authenticate Level</h2>
                <p className="text-xs font-bold text-blue-600 uppercase tracking-widest mt-1">Identity: {role?.replace('_', ' ')} @ {selectedBranch}</p>
              </div>
              <div className="space-y-4">
                <div className="relative">
                  <input 
                    ...
                    className="w-full px-8 py-5 bg-slate-50 border-2 border-slate-100 rounded-[2rem] text-sm font-black outline-none focus:border-blue-600 focus:bg-white focus:ring-4 focus:ring-blue-600/10" 
                  />
                ...
                {loginError && (
                  <div className="flex items-center gap-3 bg-red-50 p-5 rounded-3xl border border-red-100 animate-in shake duration-300">
                    <AlertCircle size={20} className="text-red-600 flex-shrink-0" />
                    <p className="text-[10px] font-black text-red-600 uppercase leading-tight">{loginError}</p>
                  </div>
                )}
              </div>
              <button type="submit" className="w-full py-6 bg-slate-950 text-white rounded-[2rem] font-black text-sm uppercase tracking-[0.3em] shadow-2xl shadow-slate-900/40 hover:bg-blue-600 transition-all">Verify Handshake</button>
            </form>
          )}
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      />
      <main className="flex-1 ml-64 p-10 relative flex flex-col min-h-screen">
        ...
        <div className="mt-auto flex flex-col md:flex-row justify-between items-center gap-6 pt-10 border-t border-slate-100">
          ...
          <div className="flex items-center gap-6">
             <p className="text-[10px] text-slate-400 uppercase tracking-widest">
               System Developed by <a href="https://mushieditz.vercel.app/" target="_blank" className="font-black text-slate-900 hover:text-blue-600 transition-colors">Mushi Editz</a>
             </p>
             <div className="w-px h-4 bg-slate-200" />
             <p className="text-[10px] text-slate-400 font-bold uppercase">© {new Date().getFullYear()} Osaka Group Holdings</p>
          </div>
        </div>
      </main>
    </div>
